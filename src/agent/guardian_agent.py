...
class LegendGuardianAgent:
    ...
    def __init__(
        self, legend_engine_url: str, legend_sdlc_url: str, api_key: str = None
    ):
    ...
    async def handle_model_change(self, event: ModelChangeEvent) -> ExecutionResult:
        ...
        try:
            logger.info(
                f"Handling model change event: {event.event_type} for model {event.model_id}"
            )
    ...
    async def _analyze_change(self, event: ModelChangeEvent) -> ChangeAnalysis:
        ...
        recommendations = self._generate_recommendations(
            event, impact_level, affected_services
        )

        analysis = ChangeAnalysis(
            event, impact_level, affected_services, recommendations
        )
    ...
    def _determine_impact_level(
        self, event: ModelChangeEvent, model_info: Dict[str, Any]
    ) -> str:
    ...
    def _generate_recommendations(
        self, event: ModelChangeEvent, impact_level: str, affected_services: list
    ) -> list:
    ...
    def _create_plan(self, analysis: ChangeAnalysis) -> ActionPlan:
        ...
        if analysis.impact_level == "critical":
            ...
            actions.extend(
                ["Service validation", "Test execution", "Deployment preparation"]
            )
        elif analysis.impact_level == "medium":
            priority = "medium"
            estimated_duration = 15
            actions.extend(
                ["Model validation", "Basic testing", "Documentation update"]
            )
        else:
            actions.extend(["Model validation", "Logging and monitoring"])

        # Add standard actions
        actions.extend(
            ["Update change log", "Monitor system health", "Generate report"]
        )

        plan = ActionPlan(analysis, actions, priority, estimated_duration)
        logger.info(
            f"Action plan created with {len(actions)} actions, priority: {priority}"
        )
    ...
    async def _execute_plan(self, plan: ActionPlan) -> ExecutionResult:
        ...
        try:
            ...
            logger.info(
                f"Plan execution completed: success={success}, time={execution_time}s, errors={len(errors)}"
            )
    ...
    async def _execute_action(
        self, action: str, analysis: ChangeAnalysis
    ) -> Dict[str, Any]:
