...
app = FastAPI(
    ...
    **FINOS Legend platform monitoring and management system**
    ...
    tags=[
        ...
        {"name": "Memory", "description": "Agent memory and event history"},
    ],
)
...
@app.post(
    "/api/v1/model/change",
    response_model=ModelChangeResponse,
    tags=["Model Management"],
)
async def handle_model_change(
    request: ModelChangeRequest, authenticated: bool = Depends(verify_api_key)
):
...
async def validate_model(
    request: ModelValidationRequest, authenticated: bool = Depends(verify_api_key)
):
...
async def get_system_status(authenticated: bool = Depends(verify_api_key)):
    ...
    try:
        sys_status = agent.get_system_status()
        return SystemStatusResponse(**sys_status)
...
async def get_recent_events(
    limit: int = 10, authenticated: bool = Depends(verify_api_key)
):
...
async def get_events_by_type(
    event_type: str, authenticated: bool = Depends(verify_api_key)
):
...
async def root():
    ...
    environment = (
        "production"
        if not os.getenv("LEGEND_API_DEBUG", "false").lower() == "true"
        else "development"
    )
...
async def get_server_info(authenticated: bool = Depends(verify_api_key)):
    ...
    try:
        ...
        return ServerInfoResponse(
            ...
            debug_mode=os.getenv("LEGEND_API_DEBUG", "false").lower() == "true",
            timestamp=datetime.now().isoformat(),
            legend_services={"engine": engine_status, "sdlc": sdlc_status},
            api_endpoints={
                "health": "/health",
                "server_info": "/api/v1/server/info",
                "model_change": "/api/v1/model/change",
