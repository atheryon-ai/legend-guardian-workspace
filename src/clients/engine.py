...
class LegendEngineClient:
    ...
    async def generate_plan(self, mapping: str, runtime: str, query: str) -> dict:
        ...
        try:
            ...
            response = await self.client.post(
                "/api/pure/v1/execution/execute", json=payload
            )
            response.raise_for_status()
            return response.json()
        except httpx.HTTPStatusError as e:
    ...
    async def transform(self, schema_type: str, class_path: str) -> dict:
        ...
        try:
            payload = {"class": class_path, "type": schema_type}
            response = await self.client.post(
                "/api/pure/v1/schema/generation", json=payload
            )
            response.raise_for_status()
            return response.json()
        except httpx.HTTPStatusError as e:
    ...
    async def run_service(self, path: str, params: dict) -> dict:
        ...
        try:
            response = await self.client.get(
                f"/api/service/v1/execute/{path}", params=params
            )
            response.raise_for_status()
            return response.json()
        except httpx.HTTPStatusError as e:
    ...
    async def generate_service(
        self, project_id: str, workspace_id: str, service_path: str, query: str
    ) -> dict:
        """Generate a service."""
        try:
            payload = {"path": service_path, "query": query}
            response = await self.client.post(
