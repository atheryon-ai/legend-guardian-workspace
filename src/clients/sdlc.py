...
class LegendSdlcClient:
    ...
    async def open_review(
        self, project_id: str, workspace_id: str, title: str, description: str
    ) -> dict:
        """Open a review for a workspace."""
        try:
            payload = {"title": title, "description": description}
            response = await self.client.post(
                f"/projects/{project_id}/workspaces/{workspace_id}/review", json=payload
            )
            response.raise_for_status()
            return response.json()
        except httpx.HTTPStatusError as e:
    ...
    async def create_version(
        self, project_id: str, version_id: str, notes: str
    ) -> dict:
        """Create a new version of a project."""
        try:
            payload = {"notes": notes}
            response = await self.client.post(
                f"/projects/{project_id}/versions/{version_id}", json=payload
            )
            response.raise_for_status()
            return response.json()
        except httpx.HTTPStatusError as e:
