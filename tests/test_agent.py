...
def test_agent_memory():
    ...
    # Create test data
    event = ModelChangeEvent(
        event_type="test", model_id="Test", timestamp=datetime.now(), details={}
    )

    analysis = ChangeAnalysis(
        event=event, impact_level="low", affected_services=[], recommendations=[]
    )

    plan = ActionPlan(
        analysis=analysis, actions=[], priority="low", estimated_duration=5
    )

    result = ExecutionResult(
        plan=plan, success=True, execution_time=1, errors=[], output={}
    )
