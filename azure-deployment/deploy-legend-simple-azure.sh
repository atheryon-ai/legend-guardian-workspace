...
# Simplified Azure Legend Platform Deployment
# This script deploys the core Legend services step by step
...
echo "🚀 Azure Legend Platform Deployment (Simplified)"
echo "================================================"
...
deploy_mongodb() {
    ...
    if kubectl get deployment mongodb -n $NAMESPACE > /dev/null 2>&1; then
        print_status "MongoDB already exists, skipping deployment"
        return
    fi
...
# Main deployment
main() {
    echo "Starting Legend Platform deployment on Azure AKS..."
    echo ""
