...
# Azure Legend Platform Deployment (Correct Images)
# This script deploys Legend using the correct FINOS image names and tags
...
echo "🚀 Azure Legend Platform Deployment (Correct Images)"
echo "==================================================="
...
# Deploy Legend Engine (Using correct image)
deploy_legend_engine() {
    print_status "Deploying Legend Engine (Using correct image)..."
...
# Deploy Legend SDLC (Using correct image)
deploy_legend_sdlc() {
    print_status "Deploying Legend SDLC (Using correct image)..."
...
# Deploy Legend Studio (Using correct image)
deploy_legend_studio() {
    print_status "Deploying Legend Studio (Using correct image)..."
...
wait_for_deployments() {
    print_status "Waiting for deployments to be ready (with longer timeout)..."
    
    # Wait for Legend services with longer timeout
    kubectl wait --for=condition=available --timeout=600s deployment/legend-engine -n $NAMESPACE
    kubectl wait --for=condition=available --timeout=600s deployment/legend-sdlc -n $NAMESPACE
    kubectl wait --for=condition=available --timeout=600s deployment/legend-studio -n $NAMESPACE
...
# Main deployment
main() {
    echo "Starting Legend Platform deployment on Azure AKS (Correct Images)..."
    echo ""
